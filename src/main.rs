...
fn main () {
    let fd: RawFd = unsafe { shm_open ("/test\0".as_ptr() as * const _, O_CREAT | O_RDWR, (S_IRUSR | S_IWUSR | S_IXUSR) as c_int) };
    println! ("fd: {fd}");

    let rs = unsafe { ftruncate (fd, 2048) };
    println! ("ftruncate: {rs}");

    let buf: * mut i8 = unsafe { mmap (ptr::null_mut(), 1024, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0) } as * mut _;
